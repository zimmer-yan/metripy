        ...
        for exit_code, failures in sorted(self.failure.items(), reverse=True):
            for failure in failures:
                if statistics[failure.value][failure.severity] >= failure.amount:
                    self.output.writeln(f"<error>Failure condition met: {failure.value} has {statistics[failure.value][failure.severity]}/{failure.amount} of severity {failure.severity}</error>")
                    exit_codes.append(exit_code)

        return max(exit_codes)

    def get_statistics(self, project_metrics_list: list[ProjectMetrics]) -> dict[str, dict[str, int]]:
        ...
        for project_metrics in project_metrics_list:
            for file_metrics in project_metrics.file_metrics:
                ...
                complexity_segment = Segmentor.get_complexity_segment(file_metrics.avgCcPerFunction)
                maintainability_segment = Segmentor.get_maintainability_segment(file_metrics.maintainabilityIndex)
                cognitive_complexity_segment = Segmentor.get_complexity_segment(file_metrics.avg_cog_complexity_per_function)
                ...
                statistics["file_cognitive_complexity"][cognitive_complexity_segment] += 1

                for class_node in file_metrics.class_nodes:
                    loc_segment = Segmentor.get_loc_segment(class_node.get_loc())
                    complexity_segment = Segmentor.get_complexity_segment(class_node.real_complexity)
                ...
                for function_node in file_metrics.function_nodes:
                    ...
                    complexity_segment = Segmentor.get_complexity_segment(function_node.complexity)
                    maintainability_segment = Segmentor.get_maintainability_segment(function_node.maintainability_index)
                    cognitive_complexity_segment = Segmentor.get_complexity_segment(function_node.cognitive_complexity)

                    statistics["function_loc"][loc_segment] += 1
                    statistics["function_cyclomatic_complexity"][complexity_segment] += 1
                    statistics["function_maintainability_index"][maintainability_segment] += 1
                    statistics["function_cognitive_complexity"][cognitive_complexity_segment] += 1
